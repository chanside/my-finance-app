...
function startQRScanner() {
  ...
  Html5Qrcode.getCameras().then(cameras => {
    ...
    document.getElementById("switch-camera-btn").style.display = cameraList.length > 1 ? "inline-block" : "none";
    } else {
      alert("找不到相機裝置。");
    }
  }).catch(err => {
...
function onScanSuccess(decodedText) {
  try {
    ...
    const match = decodedText.match(/(\d{3,5})\s*(元|TX)?/);
    if (match) {
      document.getElementById("expense").value = parseInt(match[1]);
      document.getElementById("category").value = "食";
      document.getElementById("note").value = "掃描發票";
      addTransaction();
    } else {
